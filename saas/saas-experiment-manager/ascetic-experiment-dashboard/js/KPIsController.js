...
var appmanager_host = "http://192.168.3.222/application-manager";
var eventprovider_host = "http://192.168.3.222:9000";

angular.module('asceticApp')
.factory('Applications', function($resource) {
  return $resource(appmanager_host + "/applications/",{},{
	  query: {method: 'GET', isArray:false}
  });
})
.factory('Deployments', function($resource) {
  return $resource(appmanager_host + "/applications/:appId/deployments/", { appId :"0"});
})
.factory('Events', function($resource) {
  return $resource(eventprovider_host + "/query", {
        find: {
            method: 'POST',
...
.factory('VMs', function($resource) {
  return $resource(appmanager_host + "/applications/:appId/deployments/:deploymentId/vms/", { appId :"0", deploymentId : "0"});
})
;
 ...
 */
asceticControllers.controller('KPIsCtrl', ['$q','$scope', '$http', '$routeParams','Applications','Deployments','VMs','Events',
		function ($q,$scope, $http, $routeParams,Applications,Deployments,VMs,Events) {
			...
			Applications.query(function(data) {
		        $scope.appIds = data.items.application.map(o => o.name);
			...
			$scope.selectApp = function () {
				...
				Deployments.get({appId:$scope.currentAppId}, function(data){
		            $scope.deplIds = data.items.deployment.map(o => o.id);  
		        });
			}

			$scope.kpis = [];
			d3.json("kpis.json", function (error, data) {
				if (!error) {
					console.log(data);
					d3.json("metrics.json", function (error, metrics) {
						if (!error) {
			...
			$scope.selectDepl = function () {
				for (let d of $scope.currentDeplIds) {
					if (!vmsCache.hasOwnProperty(d)) {
						$http.get(appmanager_host+ "/applications/" + $scope.currentAppId + "/deployments/" + d + "/vms/").success(function (data) {
			...
			function loadAllDataXml() {
				var deferred = Q.defer();
				$http.get("http://localhost:8000/dashboard/allthedata.xml").success(function (data) {
			...
			function loadAllEventsData() {
				return loadAllDataXml().then(function (xmldata) {
					...
					for (let deplId of $scope.currentDeplIds) {
						...
						console.log(query);
						$http.post("http://192.168.3.222:9000/query", query, {
							"headers" : {
						...
						.success(function (events) {
							...
							for (let event of events) {
								...
								for (let vm of vmsCache[490]) {
									...
									var uribase = "http://192.168.3.222/application-manager/applications/" + $scope.currentAppId + "/deployments/" + deplId + "/vms/" + vm + "/events/" + event['_id'];
