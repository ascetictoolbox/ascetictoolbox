...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/trunk/src/tracer/hwc/common_hwc.c $
 | @last_commit: $Date: 2015-04-22 14:39:08 +0200 (mié, 22 abr 2015) $
 | @version:     $Revision: 3263 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: common_hwc.c 3263 2015-04-22 12:39:08Z harald $";
