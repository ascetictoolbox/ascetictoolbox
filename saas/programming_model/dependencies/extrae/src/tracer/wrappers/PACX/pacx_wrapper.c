...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/trunk/src/tracer/wrappers/PACX/pacx_wrapper.c $
 | @last_commit: $Date: 2014-11-18 14:28:41 +0100 (mar, 18 nov 2014) $
 | @version:     $Revision: 3051 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: pacx_wrapper.c 3051 2014-11-18 13:28:41Z gllort $";
