...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/trunk/src/merger/dimemas/trace_to_trf.c $
 | @last_commit: $Date: 2015-03-13 11:14:18 +0100 (vie, 13 mar 2015) $
 | @version:     $Revision: 3181 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: trace_to_trf.c 3181 2015-03-13 10:14:18Z harald $";
