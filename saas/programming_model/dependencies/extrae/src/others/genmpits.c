...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/trunk/src/others/genmpits.c $
 | @last_commit: $Date: 2014-06-18 12:54:41 +0200 (mié, 18 jun 2014) $
 | @version:     $Revision: 2761 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: genmpits.c 2761 2014-06-18 10:54:41Z harald $";
