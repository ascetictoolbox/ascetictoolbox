...
public class MethodResourceDescription extends WorkerResourceDescription {
    ...
    public MethodResourceDescription(Constraints constraints) {
        ...
        if (queueString.compareTo(UNASSIGNED) != 0) {
            int last = 0;
            while (queueString.length() > 0) {
                last = queueString.lastIndexOf(",");
                hostQueue.add(queueString.substring(last + 1, queueString.length()));
                if (last == -1) {
                    queueString = "";
                } else {
                    queueString = queueString.substring(0, last);
                }
            }
        }
