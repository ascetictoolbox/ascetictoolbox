...
public class TaskAnalyser {
    ...
    public void processTask(Task currentTask) {
        if (drawGraph) {
            drawTask(currentTask, currentTask.getTaskParams().getType() == TaskParams.Type.METHOD);
        }

        int currentTaskId = currentTask.getId();
        Parameter[] parameters = currentTask.getTaskParams().getParameters();
        depGraph.addNode(currentTaskId, currentTask);

        // Update task count
        Integer methodId = currentTask.getTaskParams().getId();
        ...
        try {
            ...
            TaskParams c = currentTask.getTaskParams();
            if (c.hasTargetObject()) {
                ...
                if (dependingDataId != null) {
                    ...
                    if (c.getType() == TaskParams.Type.SERVICE) {
            ...
            try {
                if (!depGraph.hasPredecessors(currentTaskId)) {
                    ...
                    s.add(currentTask);
                    currentTaskCount.put(currentTask.getTaskParams().getId(), currentTaskCount.get(currentTask.getTaskParams().getId()) - 1);

                    TD.scheduleTasks(s, false, new int[CoreManager.coreCount], obsoleteRenamings);
    ...
    public void updateGraph(Task task, int implId, String resource) {
        ...
        obsoleteData(DIP.dataHasBeenRead(readedData, task.getTaskParams().getId()));
        ...
        if (!toSchedule.isEmpty()) {
            ...
            int[] successors = new int[CoreManager.coreCount];
            for (Task t : toSchedule) {
                try {
    ...
    private void drawTask(Task task, boolean isMethod) {
        ...
        switch (task.getTaskParams().getId() % 12) {
            case 0:
