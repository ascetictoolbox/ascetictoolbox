...
then
  knife bootstrap $BOOTSTRAP_IP -x root -P 'password' -N vmic-$(echo $BOOTSTRAP_IP | cut -d'.' -f 4) --bootstrap-url 'http://10.0.0.1:8080/install.sh' --no-color -s https://$(hostname):443
  if [ $? -ne 0 ]
  then
