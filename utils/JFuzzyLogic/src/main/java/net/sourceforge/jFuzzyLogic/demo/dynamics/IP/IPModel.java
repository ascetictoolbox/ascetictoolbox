...
/**
 * An IPModel object represents the dynamics of an inverted pendulum attached to
 * a sliding block.
 *
...
public final class IPModel implements Model, ForceDrivenModel {
     ...
     *
     * Runge Kutta Notes - taken from Mathematical Methods of Physics - 2nd ed
     * Mathews + Walker
     *
     * dy/dx = f(x,y) - this is the equation we know, what we want to find is
     * y(x)
     *
     * Suppose y = y0 at x = x0, we choose a small interval h and calculate
     *
     * k1 = h * f(x0, y0) k2 = h * f(x0 + h/2, y0 + k1/2) k3 = h * f(x0 + h/2,
     * y0 + k2/2) k4 = h * f(x0 + h, y0 + k3)
     *
     ...
     *
     * Note that our diffeq are not actually explicitly dependent on t - since
     * we take Fx and Fphi to be constant in our interval h. But I include t in
     * the calls I make for clarity.
     */
    ...
    public String toString() {
        ...
        return String.format(" phi = %7.1f \t dphidt= %7.1f \t x= %7.2f \t dxdt= %7.1f \t force= %7.1f", getPhiInDegrees(), getDPhiDtInDegrees(), x, dxdt, Fx);
