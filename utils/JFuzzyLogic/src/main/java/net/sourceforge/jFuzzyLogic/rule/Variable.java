...
public class Variable extends FclObject implements Comparable<Variable>, Iterable<LinguisticTerm>, CompileCpp {
    ...
    /**
     * Defuzzify this (output) variable Set defuzzufied values to 'value' and
    ...
    public LinguisticTerm getLinguisticTerm(String termName) {
        ...
        if (lt == null) {
            throw new RuntimeException("No such linguistic term: '" + termName + "'");
        }
        return lt;
    }
    ...
    public double getMembership(String termName) {
        ...
        if (mf == null) {
            throw new RuntimeException("No such termName: \"" + termName + "\"");
        }
    ...
    /**
     * Do we need to estimate universe? (e.g. when a variable referred by a
     * linguistic term changed)
     */
