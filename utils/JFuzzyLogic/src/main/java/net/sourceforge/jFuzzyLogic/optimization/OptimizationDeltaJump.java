 ...
 *
 * Tries to optimize parameters (fuzzy sets parameters and rule's weights) using
 * a 'jump' algorithm. Jump optimization: Simply tries different parameters
 * 'jumping' (changing the parameter a specified amount).
 *
 * Note: This is NOT a very sophisticated algorithm (actually looks a lot like
 * 'random search'), but if you are not in a hurry, most of the time works
...
public class OptimizationDeltaJump extends OptimizationMethod {
    ...
    /**
     * Gradient optimization for error functions Gradient descent algorithm It
     * also does a line search to get a good 'next point' on each iteration
     *
     * @param verbose : Be verbose (recomended, because it takes some time to
     * converge)
     */
    @Override
    public void optimizeIteration(int iterationNum) {
        ...
        for (paramNum = 0; paramNum < len; paramNum++) {
            ...
            if (jump != 0) { // Can jump?
                ...
                for (lineIterNum = 0; lineIterNum < maxLineSearchIterations; lineIterNum++) // Advance towards 'jump' direction (step = alpha * jump)
                {
