...
public class DemoPanelFis extends JPanel {
    ...
    public JFreeChart chart(Variable v) {
        ...
        if (!discrete) {
            chart = ChartFactory.createXYAreaChart(topTitle, "x", "Membership", xyDataset, PlotOrientation.VERTICAL, true, true, false);
        } else {
            chart = ChartFactory.createScatterPlot(topTitle, "x", "Membership", xyDataset, PlotOrientation.VERTICAL, true, true, false); // Set 'Value' color to BLACK 
        }
