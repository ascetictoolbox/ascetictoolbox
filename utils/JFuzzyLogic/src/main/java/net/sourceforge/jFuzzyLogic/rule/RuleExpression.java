...
public class RuleExpression extends FclObject implements Iterable<Variable>, CompileCpp {
     ...
     * @param ruleConnectionMethod : connection method between terms (which
     * method is used for each 'and', 'or', 'not'...)
     */
    ...
    public String toStringCpp() {
        ...
        if ((term1 == null) || (term2 == null)) {
            // Only one term?
            if (term1 != null) {
                str += term1.toStringCpp();
            }
            if (term2 != null) {
                str += term2.toStringCpp();
            }
        } else {
            str += connector + "(";
            ...
            if (isFuzzyRuleExpression(term1)) {
                str += ((RuleExpression) term1).toStringCpp();
            } else if (isFuzzyRuleTerm(term1)) {
                str += ((RuleTerm) term1).toStringCpp();
            }

            str += " , ";

            if (isFuzzyRuleExpression(term2)) {
                str += ((RuleExpression) term2).toStringCpp();
            } else if (isFuzzyRuleTerm(term2)) {
                str += ((RuleTerm) term2).toStringCpp();
            }
