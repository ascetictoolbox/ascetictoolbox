...
/**
 * Generic continuous defuzzifier (a defuzzifier for continuous membership
 * functions)
 *
...
public abstract class DefuzzifierContinuous extends Defuzzifier {
    ...
    /**
     * Step size between each element in 'values[]' stepSize = (max - min) /
     * values.length
     */
    protected double stepSize;

    /**
     * Funcion values: A generic continuous function y = f(x) where x : [min,
     * max] Values are stored in 'values[]' array. Array's index is calculated
     * as: index = (x - min) / (max - min) * (values.length)
     */
