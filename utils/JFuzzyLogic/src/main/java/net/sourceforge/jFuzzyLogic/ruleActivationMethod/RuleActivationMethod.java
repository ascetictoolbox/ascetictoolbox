...
/**
 * Rule activation method Base abstract class
 *
 * From FCL's definition: "In the conclusion, subconclusions relate to the
 * output variables. The degree of membership of the conclusion is then
 * determined on the basis of the degree of accomplishment of the condition
 * determined in aggregation (Conclusion IF A THEN B). In general, the MIN or
 * PROD is used for activation. If the rule base contains rules with weighting
 * factors wk, where wk in [0, 1], this can be implemented by means of
 * multiplication."
 *
...
public abstract class RuleActivationMethod extends FclObject {
    ...
    /**
     * Inference method Add membershipfunction to deffuzifier (using 'min' as
     * inference)
     */
    ...
    public String toStringCpp(RuleTerm fuzzyRuleTerm, RuleAccumulationMethod ruleAccumulationMethod, String degreeOfSupportName) {
        ...
        if (fuzzyRuleTerm.getMembershipFunction().isDiscrete()) {
            ...
            // Is term negated?
            if (fuzzyRuleTerm.isNegated()) {
                out.append("\t\tmembership = 1 - mf.membership(x);\n");
            } else {
