...
public abstract class Defuzzifier extends FclObject {
    ...
    /**
     * Deffuzification function Note: Has to return Double.NaN if no rule
     * inferred this variable
     */
    ...
    /**
     * Do we need to re-initialize? E.g. Defuzzified depends on an input
     * variable that changed value
     */
    public abstract boolean needsInit();

    /**
     * Reset defuzzifier values, this method is invoked on every
     * RuleSet.evaluate()
     */
    public void reset() {
        if (needsInit()) {
