...
public class MembershipFunctionPieceWiseLinear extends MembershipFunctionContinuous {
     ...
     * @param y [] : y points array A piecewise linear function is defined by
     * 'n' points: (x_1,y_1) , (x_2,y2) , ... (x_n,y_n) ordered by x[]
     * (increasing) See also 'membership()' for a precise definition.
     */
    ...
    /**
     * Need to override this method (we store parameters differently in this
     * function)
     */
    @Override
    ...
    /**
     * Membership funcion is defined as: membership(x) = y[0]	if x <= x[0]
     * membership(x) = y[n]	if x >= x[n] (where n = x.length) membership(x) =
     * y[i - 1] + (y[i] - y[i - 1]) / (in - x[i])	if x[i-1] < x <= x[i]
     *
     * @see
     * net.sourceforge.jFuzzyLogic.membership.MembershipFunction#membership(double)
     */
    @Override
