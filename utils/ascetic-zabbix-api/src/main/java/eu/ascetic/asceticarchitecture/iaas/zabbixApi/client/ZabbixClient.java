...
//						Item item = Json2ObjectMapper.getStringFormatItem((HashMap<String, String>) result.get(i));
						items.add(item);
					}
				}
				return items;
...
//								Item item = Json2ObjectMapper.getStringFormatItem((HashMap<String, String>) result.get(i));
	 ...
	 * @param hostName the host name
	 * @param isFormatDouble true is item format is double, false if item format is String
	 * @return the item from host
	 */
	public Item getItemByNameFromHost(String itemName, String hostName, boolean isFormatDouble){
		Item item = null;
...
//						if (isFormatDouble){
//							item = Json2ObjectMapper.getDoubleFormatItem((HashMap<String, String>) result.get(0));
//						}
//						else {
//							item = Json2ObjectMapper.getStringFormatItem((HashMap<String, String>) result.get(0));
//						}
	...
	public List<HistoryItem> getHistoryDataFromItem(String itemKey, String hostName, String itemFormat, int limit){
		...
		if (limit > 0){
			...
			if (host != null){
				//get itemId
//				boolean isDoubleFormat = true;
//				if (itemFormat.equalsIgnoreCase(Dictionary.HISTORY_ITEM_FORMAT_STRING) || 
//						itemFormat.equalsIgnoreCase(Dictionary.HISTORY_ITEM_FORMAT_TEXT) ||
//						itemFormat.equalsIgnoreCase(Dictionary.HISTORY_ITEM_FORMAT_LOG)){
//					isDoubleFormat = false;
//				}
//				Item item = getItemByKeyFromHost(itemKey, hostName, isDoubleFormat);
  ...
  /**
    ...
    public List<HistoryItem> getHistoryDataFromItem(String itemKey, String hostName, String itemFormat, long startTime,
        ...
        if (startTime <= endTime) {
                ...
                if (host != null) {
                    //get itemId
//                    Item item = getItemByKeyFromHost(itemKey, hostName, false);
                	 Item item = getItemByKeyFromHost(itemKey, hostName);
                    if (item != null) {
                        try {
                            String token = getAuth();
	...
	/**
	 * Gets the templates of host.
	 *
	 * @param hostName the host name
	 * @return the templates of host
	 */
	public String[] getTemplatesOfHost(String hostName){
		Host h = null;
		...
		if (h!=null){
			...
			try {
				...
				if (token != null){
					...
					if (result != null){
						//[{hostid=10116, parentTemplates=[{templateid=10091, name=Template Virt VMware Hypervisor}]}]
						HashMap<String, Object> hm = (HashMap<String, Object>) result.get(0);
						ArrayList<HashMap<String, String>> templates = (ArrayList<HashMap<String, String>>) hm.get(Dictionary.ZABBIX_TEMPLATES_ATTRIBUTE_PARENT_TEMPLATES);
						//[{templateid=10091, name=Template Virt VMware Hypervisor}]
						templatesName = new String[templates.size()];
						for (int i=0; i<templates.size(); i++){
							templatesName[i] = templates.get(i).get(Dictionary.ZABBIX_TEMPLATES_NAME);
						}
						 
						log.debug("Host " + h.getHostid() + " finded in Zabbix");
			...
			log.error("Hostname " + hostName + " not finded in Zabbix");
		}
		return templatesName;
	}
...
//	}
		...
		try {
			...
			if (token != null){
				...
				if (vmsHostGroup != null){
					String jsonRequest = 
							"{\"jsonrpc\":\"" + Dictionary.JSON_RPC_VERSION + "\","
							+ "\"params\":{\"output\":\"extend\",\"groupids\":\"" + vmsHostGroup.getGroupId() + "\"},"
							+ "\"method\":\"host.get\","
							+ "\"auth\":\""+ token + "\",\"id\": 0}";
	...
	public Item getPowerFromVM(String itemKey, String hostName){
		Item item = null;
				...
				if (token != null){
					...
					if (result != null){
						...
						while (!found && i<result.size()){
							item = Json2ObjectMapper.getItem((HashMap<String, String>) result.get(i));
							if (item.getName().equalsIgnoreCase(itemKey)){
								found = true;
							}
							i++;							
						}
