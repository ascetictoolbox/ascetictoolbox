...
public class HostFilterTest {
    ...
    public void filter() {
        ...
        hosts.add(new HostFake("host1", 4, 4096, 4, 1, 1024, 1)); // OK
        hosts.add(new HostFake("host2", 4, 4096, 4, 3, 1024, 1)); // not enough CPUs
        hosts.add(new HostFake("host3", 4, 4096, 4, 1, 3072, 1)); // not enough RAM
        hosts.add(new HostFake("host4", 4, 4096, 4, 1, 1024, 3)); // not enough disk

        // Filter hosts and check that only host1 is in the result
        List<Host> filteredHosts = HostFilter.filter(hosts, 2, 2048, 2);
