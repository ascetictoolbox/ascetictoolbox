...
public class RequestHelper implements Params.VMCharacteristic {
	...
	protected void update(String vm, Hashtable<String, String> params) {
		...
		if (agreementTerm != null) {
			...
			System.out.println("Unsupported VM Type :: " + vm);
	}
	...
	public void negotiation(SLATemplate newTemplate) {
		try {
			...
			System.out.println(String.format("SLA.AgreedAt='%s' SLA.EffectiveFrom='%s' SLA.EffectiveUntil='%s'", sla.getAgreedAt(), sla.getEffectiveFrom(), sla.getEffectiveUntil()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
