...
//	
//	private static String QUERY_AGG_HOUR_TIME = "select YEAR(FROM_UNIXTIME(vmm.clock)) as yeart, MONTH(FROM_UNIXTIME(vmm.clock)) as mon, DAYOFMONTH(FROM_UNIXTIME(vmm.clock)) as dayt, HOUR(FROM_UNIXTIME(vmm.clock)) as hh, avg(energy)as energy ,avg(power)as power, avg(cpu_load) as loadvm from vm_measurement as vmm JOIN host_measurement as hmm on hmm.host_id=vmm.host_id and vmm.clock = hmm.clock where hmm.host_id = ? and vmm.vm_id = ? and hmm.clock > ? group by yeart,mon,dayt,hh";
//	
//
//	
//	
...
//
//	@Override
//	public List<VMConsumptionPerHour> getEnergyForVMHourlyTime(String hostid,String vmid, Timestamp time) {
//		LOGGER.info("getting vm consumption for "+hostid + " after "+time);
//		List<VMConsumptionPerHour> data = jdbcTemplate.query(QUERY_AGG_HOUR_TIME,new Object[] { hostid,vmid,time.getTime() },new VMHourlyConsumptionMapper());
//		return data;
//	}
//	
//	@Override
