...
public interface DataConsumptionMapper {
	...
	//application id | deployment id | start time | end time | event load (cpu/ram usage) | total energy consumed | min | max | avg
	  ...
	  @Select("SELECT * FROM DATACONSUMPTION WHERE deploymentid = #{deploymentid} and vmid = #{vmid} order by time asc")
	  ...
	  @Select("select IFNULL(max(time),0) from DATACONSUMPTION WHERE deploymentid = #{deploymentid} and vmid = #{vmid} and time <= #{time}")
