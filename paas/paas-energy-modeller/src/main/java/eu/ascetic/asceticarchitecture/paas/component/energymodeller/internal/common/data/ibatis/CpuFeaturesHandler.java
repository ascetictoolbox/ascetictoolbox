 ...
 * 
 * @author M. Fontanella
 * build a connection with the database to retrieve data about CPU features (tdp, minpower, maxpower)
 */
public class CpuFeaturesHandler {

	private static SqlSessionFactory sqlSessionFactory;
	private static CpuFeaturesHandler instance;
	
	private final static Logger LOGGER = Logger.getLogger(CpuFeaturesHandler.class.getName());
	
	
	private CpuFeaturesHandler(String driver,String url,String uname,String pwd){
		...
		configuration.addMapper(CpuFeaturesMapper.class);
		sqlSessionFactory = new SqlSessionFactoryBuilder().build(configuration);
		LOGGER.info("Data collector set up complete");
	}
	
	public static CpuFeaturesHandler getHandler(String driver,String url,String uname,String pwd){
		LOGGER.info("Returning the handler");
		if (instance==null) {
			instance = new CpuFeaturesHandler(driver,url,uname,pwd);
