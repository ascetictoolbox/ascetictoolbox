...
public class LibvirtControlBridge implements LibvirtControlBridgeMBean {
	 ...
	 * 
	 * @param connection
	 *            The connection to use when sending commands
	 */
	...
	/* (non-Javadoc)
	 * @see eu.ascetic.vmc.api.core.libvirt.LibvirtControlBridgeMBean#attachIso(java.lang.String, java.lang.String, java.lang.String)
	 */
	@Override
	public void attachIso(String domainName, String isoPath,
			String interfaceName) throws LibvirtException {

		Domain domain = connection.domainLookupByName(domainName);

		LOGGER.info("Running detachIso on domainName: " + domainName
				+ ", isoPath: " + isoPath + "interfaceName: " + interfaceName);
		...
		if (!isXenRunning(domain)) {
			...
			// xm block-attach deb-squeeze.qcow2.hvm
			// file:/mnt/glusterfs-nfs/images/user-images/django/recontext.iso
			// hdd:cdrom r
			try {
	...
	/* (non-Javadoc)
	 * @see eu.ascetic.vmc.api.core.libvirt.LibvirtControlBridgeMBean#detachIso(java.lang.String, java.lang.String)
	 */
	@Override
	public void detachIso(String domainName, String interfaceName)
	 ...
	 * @param interFaceName The name of the interface where the ISO should be
	 * @param attaching True when attaching, false when detaching
	 * @return The XML representation of the ISO
	 */
	private String getIsoXML(String isoPath, String interFaceName,
			boolean attaching) {
	...
	/* (non-Javadoc)
	 * @see eu.ascetic.vmc.api.core.libvirt.LibvirtControlBridgeMBean#getExistingIsoPath(java.lang.String, java.lang.String)
	 */
	@Override
	public String getExistingIsoPath(String domainName, String interfaceName)
			throws LibvirtException {
		Domain domain = connection.domainLookupByName(domainName);
	...
	/**
	 * Utility function to parse disk filename from libvirt XML
	 * 
	 * @param xmlDesc
	 *            The device XML description
	 * @return the fileName of the disk
	 */
	private String parseXML(String xmlDesc) {

		String deviceBlock = xmlDesc.substring(xmlDesc.indexOf("<devices>"),
				xmlDesc.indexOf("</devices>"));
		int startIndex = 0;
		while (true) {
			int diskStartIndex = deviceBlock.indexOf("<disk", startIndex);
			int diskEndIndex = deviceBlock.indexOf("</disk>", diskStartIndex);

			String diskBlock = deviceBlock.substring(diskStartIndex,
					diskEndIndex);
			...
			if (diskBlock.contains(DomainContextualizer.CONTEXT_INTERFACE_NAME)) {
				...
				String fileBlock = diskBlock.substring(sourceIndex,
						diskBlock.indexOf("/>", sourceIndex));
				LOGGER.debug("fileBlock: " + diskBlock);
				String fileName = fileBlock.substring(
						fileBlock.indexOf('\'') + 1,
						fileBlock.lastIndexOf('\''));
				LOGGER.debug("fileName: " + diskBlock);
				return fileName;
			} else {
				LOGGER.debug("Diskblock doesn't contain: "
						+ DomainContextualizer.RECONTEXT_INTERFACE_NAME);
			}
	...
	/**
	 * Main method to test control bridge functionality.
	 * 
	 * @param args
	 */
