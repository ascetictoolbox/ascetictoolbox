...
public class ServiceManifestClient {
	 ...
	 */
	public ServiceManifestClient(OvfDefinition ovfDefinition) {
	 ...
	 */
	public OvfDefinition addIsosToManifest(
	 ...
	 * @return The altered ovfDefinition.
	 */
	public OvfDefinition addHardDisksToManifest(
			Map<String, VirtualMachine> virtualMachines) {
		...
		for (VirtualMachine virtualMachine : virtualMachines.values()) {
			...
			for (HardDisk hardDisk : hardDisks.values()) {
				// FIXME: Manifest only supports a single image, this should
				// change...
