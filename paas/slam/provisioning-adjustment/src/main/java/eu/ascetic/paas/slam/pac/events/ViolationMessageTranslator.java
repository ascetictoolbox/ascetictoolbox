...
import eu.ascetic.paas.slam.pac.events.converter.ValueConverter;
...
public class ViolationMessageTranslator extends EventTranslator {
	...
	public Message fromXML(String messageStr) {
		...
		xstream.registerConverter(new eu.ascetic.paas.slam.pac.events.converter.DateTimeConverter());

		xstream.registerConverter(new ValueConverter());

		xstream.alias(ViolationMessage.class.getSimpleName(), ViolationMessage.class);
		xstream.useAttributeFor(ViolationMessage.class, "appId");
		xstream.useAttributeFor(ViolationMessage.class, "deploymentId");
